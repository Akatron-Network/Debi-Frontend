...
export default function Favorites(props) {
  const { allFavorites, favoriteFile, collections, funcLoadForSpesific, gatewayClientCheck } = useContext(MainContext);
				...
				{allFavorites.map((f, i) => {
					...
					for (let c of collections) {
						if (f.page.collection_id === c.collection_id) {
							...
							if (c.connector.gateway_host !== null && !gatewayClientCheck) {
								return (
								<div key={i} className="tooltip tooltip-bottom" data-tip="İstemci Gerektirir">
									<div className='relative opacity-30 bg-danger'>
										...
										<Link to={f.url} className='mb-1 pointer-events-none '>
